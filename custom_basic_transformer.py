...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is None:
            mask = torch.triu(torch.ones((b, n, n), device=x.device), diagonal=1).bool()
            mask = ~mask
        else:
            # Expand existing mask for multiple heads
            mask = mask.squeeze(1).unsqueeze(1).expand(-1, h, -1, -1)
...
class AutoregressiveWrapper(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        # Replace any ignored indices with pad value in input
        ...
        loss = F.cross_entropy(
            rearrange(logits, 'b n c -> b c n'),  # Reshape for cross_entropy
            target,
